...
Ext.define('Ext.ux.grid.FiltersFeature', {
    ...
    createFilters: function() {
        ...
        // Note that we will not recreate filters that are defined in the filters feature config block when
        // reconfiguring the grid, but we must recreate any filters that are defined on the columns themselves.
    ...
    onAddRemoveColumn: function (headerCt, column) {
        // only need to change filters if we are adding/removing a column with filters
        if (column.filter) {
            this.createFilters();
        }
    },
    ...
    updateColumnHeadings: function (headerCt) {
        ...
        headerCt = headerCt || me.view.headerCt;
        ...
        if (headerCt) {
            headerCt.items.each(function (header) {
                ...
                filter = me.getFilter(header.dataIndex);
