...
Ext.define('PageAnalyzer.MainForm', {
    ...
    getTopLevelComponents: function () {
        var me = this,
            all = me.getTargetExt().ComponentManager.all.getArray(),
            top = [];
    ...
    onGatherStats: function() {
        var me = this,
            target = me.target,
            targetExt = me.getTargetExt(),
            perf = targetExt.Perf,
            con = me.viewport.down('#perfcon');

        // Only gather page startup stats at each reload, not on each gather
        if (!me.statsGatherCount) {
            perf.get("Initialize").enter().leave(targetExt._afterReadytime - targetExt._beforeReadyTime);
            if (targetExt._endTime) {
                perf.get("Load").enter().leave(targetExt._endTime - targetExt._startTime);
                perf.get("WaitForReady").enter().leave(targetExt._readyTime - targetExt._endTime);
            } else {
                perf.get("BeforeReady").enter().leave(targetExt._readyTime - targetExt._startTime);
            }
        }
    ...
    getTargetExt: function() {
        var target = this.target;
        return target && (target.Ext4 || target.Ext);
    },

    getState: function() {
        var states = this.states,
            target = this.target,
            targetExt = this.getTargetExt();

        if (targetExt && targetExt.isReady) {

            if (target._layoutRuns && targetExt._readyTime) {
                return states.ready;
            }
            if (target._hooking) {
    ...
    updateConnectedState: function() {
        ...
        if (state == states.loaded) {
            ...
            if (me.reloading) {
                ...
                me.reloading = false;
                targetExt._continueFireReady = true;
                targetExt.EventManager.readyEvent.fire();
            }
            me.updateLayoutRuns();
        }
    }
